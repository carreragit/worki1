...
public class EmailService {
    ...
    public void enviarVerificacionEmail(String email, String token) {
        String link = System.getenv().getOrDefault("GATEWAY_URL", "http://localhost:8080") + "/api/auth/verificar-email?token=" + token;
        System.out.println("[EMAIL] Verificación para " + email + ": " + link);
    }

    public void enviarRecuperacionPassword(String email, String token) {
        String link = System.getenv().getOrDefault("GATEWAY_URL", "http://localhost:8080") + "/api/auth/reset-password?token=" + token;
